...
keywords %>% group_by(year) %>% 
  arrange(desc(count), .by_group = T) %>% 
  top_n(20) %>% streamgraph(key = "keywords", value = "count", date = "year", height = "300px", width = "1000px", offset = "zero", interactive = FALSE) %>% 
  sg_fill_brewer("PuOr")

# interpolate
keywords %>% group_by(year) %>% 
  arrange(desc(count), .by_group = T) %>% 
  top_n(20) %>% streamgraph(key = "keywords", value = "count", date = "year", height = "300px", width = "1000px", offset = "zero", interactive = FALSE, interpolate = "cardinal") %>%
  sg_fill_brewer("PuOr")
...
# Streamgraphs using ggstream 

```{r}
keywords %>% group_by(year) %>% 
  arrange(desc(count), .by_group = T) %>% 
  top_n(10) %>%
  ggplot(aes(x = year, y = count, fill = keywords)) +
  geom_stream(type = "ridge", show.legend = FALSE, na.rm = TRUE, sorting = "onset") + 
  geom_stream_label(aes(label = keywords)) 
```
